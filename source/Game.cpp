...
void Game::Initialize(HWND window, int width, int height)
{
    ...
    crystalSound = new Sound3D(m_audEngine.get(), "Assets/Music/crystalSound.wav", m_listener, lighthouse.getCrystalPosition(), 5, 0);
    crystalSound->setCamera(&m_Camera01);

    soundWave = new WavesSound(m_audEngine.get());
    soundWave->setCamera(&m_Camera01);
}
...
void Game::Update(DX::StepTimer const& timer)
{
    ...
    if (updateTimer == 0)
        m_Camera01.setPosition(DirectX::SimpleMath::Vector3(0,-10,0));
    else if(updateTimer == 1)
        m_Camera01.setPosition(DirectX::SimpleMath::Vector3(0, 1, 4));
    ...
    soundWave->update(deltaTime, &m_gameInputCommands);

    crystalSound->update(deltaTime, &m_gameInputCommands);
...
void Game::CreateDeviceDependentResources()
{
   ...
   // testSystem = new ParticleSystem(device, 128, &m_world, &m_view, &m_projection);
